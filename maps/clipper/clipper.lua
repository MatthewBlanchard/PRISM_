...
function mt:__eq(p) return self.X == p.X and self.Y == p.Y end
...
local function wrap_vector(name, vecargtype)
   ...
   function vector_mt:__index(k) return type(k) == "number" and methods.at(self, k) or methods[k] end
...
local PolyNode_index = function(self, k)
   ...
   elseif k == "Childs" then
      return C["ClipperLib_PolyNode_Childs__get"](self)
   elseif k == "Parent" then
      return C["ClipperLib_PolyNode_Parent__get"](self)
   else
      return PolyNode_methods[k]
   end
end
...
local PolyTree_index = function(self, k)
   if k == "Contour" then
      ...
      return setmetatable { data = path, M.Path }
   elseif k == "Childs" then
      return C["ClipperLib_PolyTree_Childs__get"](self)
   elseif k == "Parent" then
      return C["ClipperLib_PolyTree_Parent__get"](self)
   else
      return PolyTree_methods[k]
   end
end
...
function Clipper_methods:AddPath(pg, PolyTyp, Closed)
   return C["ClipperLib_Clipper_AddPath"](self, pg.data, PolyTyp, Closed)
end
function Clipper_methods:AddPaths(ppg, PolyTyp, Closed)
   return C["ClipperLib_Clipper_AddPaths"](self, ppg.data, PolyTyp, Closed)
end
...
function Clipper_methods:PreserveCollinear(value)
   if value then
      return C["ClipperLib_Clipper_PreserveCollinear__set"](self, not not value)
   else
      return C["ClipperLib_Clipper_PreserveCollinear__get"](self)
   end
end
function Clipper_methods:Execute(clipType, solution, subjFillType, clipFillType)
   ...
   if getmetatable(solution) == M.Paths then
      return C["ClipperLib_Clipper_Execute__Paths"](
         self,
         clipType,
         solution.data,
         subjFillType,
   ...
   elseif ffi.istype("cl_PolyTree", solution) then
      return C["ClipperLib_Clipper_Execute__PolyTree"](
         self,
         clipType,
         solution,
         subjFillType,
...
function Clipper_methods:ReverseSolution(value)
   if value then
      return C["ClipperLib_Clipper_ReverseSolution__set"](self, not not value)
   else
      return C["ClipperLib_Clipper_ReverseSolution__get"](self)
   end
end
function Clipper_methods:StrictlySimple(value)
   if value then
      return C["ClipperLib_Clipper_StrictlySimple__set"](self, not not value)
   else
      return C["ClipperLib_Clipper_StrictlySimple__get"](self)
   end
end
...
function ClipperOffset_methods:AddPath(path, joinType, endType)
   return C["ClipperLib_ClipperOffset_AddPath"](self, path.data, joinType, endType)
end
function ClipperOffset_methods:AddPaths(paths, joinType, endType)
   return C["ClipperLib_ClipperOffset_AddPaths"](self, paths.data, joinType, endType)
end
...
local ClipperOffset_index = function(self, k)
   if k == "MiterLimit" then
      return C["ClipperLib_ClipperOffset_MiterLimit__get"](self)
   elseif k == "ArcTolerance" then
      return C["ClipperLib_ClipperOffset_ArcTolerance__get"](self)
   else
      return ClipperOffset_methods[k]
   end
end

local ClipperOffset_newindex = function(self, k, v)
   if k == "MiterLimit" then
      return C["ClipperLib_ClipperOffset_MiterLimit__set"](self, v)
   elseif k == "ArcTolerance" then
      return C["ClipperLib_ClipperOffset_ArcTolerance__set"](self, v)
   end
end

ffi.metatype(
   "cl_ClipperOffset",
   { __index = ClipperOffset_index, __newindex = ClipperOffset_newindex }
)
...
M.MinkowskiDiff = function(poly1, poly2, solution)
   return C.ClipperLib_MinkowskiDiff(poly1.data, poly2.data, solution.data)
end
M.PolyTreeToPaths = function(polytree, paths)
   return C.ClipperLib_PolyTreeToPaths(polytree, paths.data)
end
M.ClosedPathsFromPolyTree = function(polytree, paths)
   return C.ClipperLib_ClosedPathsFromPolyTree(polytree, paths.data)
end
M.OpenPathsFromPolyTree = function(polytree, paths)
   return C.ClipperLib_OpenPathsFromPolyTree(polytree, paths.data)
end
