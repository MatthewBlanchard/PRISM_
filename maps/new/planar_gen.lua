...
=======
	...
	local j = #polygon
	for i = 1, #polygon do
		if polygon[i].x == point.x and polygon[i].y == point.y then
			return -1
		end
...
function Level:create(callback)
   ...
   edges.empty = {
      type = "Join",
      callback = function(chunk, info) print(#info.points) end,
   }
   edges.narrow = {
      type = "Join",
      callback = function(chunk, info)
   ...
   local function graph_writer(graph)
      ...
      local shapers = {
         ...
         funnel = function(self, chunk) end,
         DLA1 = function(self, chunk, info) chunk:DLAInOut() end,
         DLA2 = function(self, chunk, info) chunk:DLA() end,
      ...
      local populaters = {
         center = function(self, info, id, callback, uuid, meta)
            ...
            map:insert_entity(
               id,
               center.x + meta.offset.x,
               center.y + meta.offset.y,
               callback,
               uuid
            )
         end,
         random = function(self, info, id, callback, uuid)
            ...
            until map:get_cell(x + offset.x, y + offset.y) == 0
               and point_in_polygon(vec2(x, y), polygon) == 1
         ...
         perimeter = function(self, info, id, callback, uuid)
            ...
            for x, y, cell in chunk:for_cells() do
               if
                  map:get_cell(x + offset.x, y + offset.y) == 1
                  and point_in_polygon(vec2(x, y), polygon) == -1
               then
         ...
         heat_map = function(self, info, id, callback, uuid)
            ...
            for i, v in ipairs(path.points) do
               if i ~= 1 and i ~= #path.points then table.insert(heat_path, v - offset) end
            end
            ...
            for x, y, cell in heat_map:for_cells() do
               if cell ~= math.huge then max = math.max(max, cell) end
            end

            local treasure_spots = {}
            for x, y, cell in heat_map:for_cells() do
               if cell >= max - max * 0.3 and cell ~= math.huge then
      ...
      local stack = {}
      function stack.push(v) table.insert(stack, v) end
      function stack.pop() return table.remove(stack) end
      ...
      local function clarify_level_specifications(level_specifications)
         chunks.add_chunk(
            "start",
            graph:add_vertex(Chunk:extend()):specify {
               width = 3,
               height = 3,
   ...
   function chunks.newFiller:populater() end
	...
	local function graph_writer(graph)
		...
		local shapers = {
			...
			tunnel = function(self, chunk, info)
				...
				local path = chunk:drunkWalk(center.x, center.y, function(x, y, i, chunk)
					return (x == 0 or x == chunk.width or y == 0 or y == chunk.height)
				end)
