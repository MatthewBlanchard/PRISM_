...
effects.throw = function(thrown, thrower, location)
   local line, valid =
      Bresenham.line(thrower.position.x, thrower.position.y, location.x, location.y)
   local lineIndex = 1
   local t = 0
   ...
   return function(dt, interface)
      ...
      for x, yt in pairs(fov) do
         for y, _ in pairs(yt) do
            ...
            elseif fade > 0.95 then
            elseif fade > 0.75 then
