...
local function endsWith(str, ending) return ending == "" or str:sub(-#ending) == ending end
...
local function inflector(mode)
   ...
   local function rule(singular, plural)
      local rulefunc = function(word)
         local s = singular .. "$"
         if string.find(word, s) then return string.gsub(word, s, plural) end
      ...
      local rulefunc = function(word)
         ...
         for i, suffix in ipairs(list) do
            if endsWith(lword, suffix) then
               if not endsWith(lword, singular) then error "Internal error!" end
