...
function Text.tokenize(str, maxWidth)
	...
	if #part > 0 then result[#result + 1] = {
		type = Text.TYPE_TEXT,
		value = part,
	} end
		...
		repeat
			...
			if lineLength + #token.value > maxWidth then
				...
				elseif lastTokenWithSpace then
					...
					token.value = Text._breakInsideToken(tokens, lastTokenWithSpace, breakIndex, true)
					i = lastTokenWithSpace
				else -- force break in this token
					token.value = Text._breakInsideToken(tokens, i, maxWidth - lineLength + 1, false)
				end
