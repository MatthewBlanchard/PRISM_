...
function Point:equals(other)
   ...
   if
      other == nil
      or not other.isPoint
      or (other.x and other.x ~= self.x)
      or (other.y and other.y ~= self.y)
   then
      return false
   end
   return true
...
function Point:equals(other)
	...
	if other == nil or not other.isPoint or (other.x and other.x ~= self.x) or (other.y and other.y ~= self.y) then
		return false
	end
	return true
end
...
function Line:getPoints()
	...
	while true do
		table.insert(self.points, Point(self.x1, self.y1))
		if self.x1 == self.x2 and self.y1 == self.y2 then
			break
		end
		local e2 = err * 2
		if e2 > -dx then
...
<<<<<<< HEAD
   ...
   for _, p in pairs(thePoints) do
      ...
      for i = 2, #line.points do
         ...
         if not self:_lightPasses(point.x, point.y) then break end
      end
   end
...
=======
	...
	for _, p in pairs(thePoints) do
		...
		for i = 2, #line.points do
			local point = line.points[i]
			if self:_oob(cx - point.x, cy - point.y, r) then
				break
			end
			if notvisited[point:hashCode()] then
				callback(point.x, point.y, i, 1 - (i / r))
