...
<<<<<<< HEAD
   ...
   if self._options.nocorridorsmode then self._features.Corridor = nil end
   ...
   repeat
      ...
      if dir then
         ...
         for i = 1, #self._walls do
            ...
            if wall.value > 1 then priorityWalls = priorityWalls + 1 end
         end
      end
...
function Digger:_priorityWallCallback(x, y) self:setWall(x, y, 2) end
...
function Digger:_tryFeature(x, y, dx, dy)
   ...
   if not feature:isValid(self._isWallCallback, self._canBeDugCallback) then return false end
...
function Digger:_getDiggingDirection(cx, cy)
	if cx < 2 or cy < 2 or cx > self._width - 1 or cy > self._height - 1 then
		return nil
	end
	local deltas = ROT.DIRS.FOUR
	local result = nil
