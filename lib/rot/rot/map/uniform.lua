...
function Uniform:_connectRooms(room1, room2)
   ...
   if start[index] >= min and start[index] <= max then
      ...
      elseif dirIndex2 == 2 then
         value = room2:getRight() + 1
      elseif dirIndex2 == 3 then
         value = room2:getBottom() + 1
      elseif dirIndex2 == 4 then
   ...
   elseif start[index] < min - 1 or start[index] > max + 1 then
      ...
      local rotation = 0
      if dirIndex2 == 1 or dirIndex2 == 2 then
         rotation = diff < 0 and 2 or 4
      elseif dirIndex2 == 3 or dirIndex2 == 4 then
         rotation = diff < 0 and 4 or 2
      end
   ...
   if index > 0 then table.insert(self._connected, table.remove(self._unconnected, index)) end
...
function Uniform:_placeInWall(room, dirIndex)
   ...
   for i = 1, length do
      ...
      if self._map[x][y] == 1 then --is a wall
         if lastBadIndex ~= i - 1 then avail[i] = { x, y } end
      else
         lastBadIndex = i
...
=======
	...
	while true do
		local t2 = os.clock() * 1000
		if t2 - t1 > self._options.timeLimit then
			return nil
		end
		self._map = self:_fillMap(1)
		self._dug = 0
		...
		self._unconnected = {}
		self:_generateRooms()
		if self:_generateCorridors() then
			break
		end
	end
...
function Uniform:_generateRooms()
	...
	repeat
		room = self:_generateRoom()
		if self._dug / (w * h) > self._options.roomDugPercentage then
			break
		end
	until not room
end
...
function Uniform:_generateCorridors()
	...
	while cnt < self._corridorAttempts do
		...
		while true do
			...
			local room1 = self:_closestRoom(self._unconnected, connected)
			local room2 = self:_closestRoom(self._connected, room1)
			if not self:_connectRooms(room1, room2) then
				break
			end
			if #self._unconnected < 1 then
				return true
			end
		end
	end
