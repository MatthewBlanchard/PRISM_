...
<<<<<<< HEAD
function Cellular:set(x, y, value) self._map[x][y] = value end
=======
...
<<<<<<< HEAD
   ...
   if self._options.connected then self:_completeMaze() end
   if callback then
      for i = 1, self._width do
         for j = 1, self._height do
...
function Cellular:_completeMaze()
   ...
   for i = 1, self._width do
      for j = 1, self._height do
         if self._map[i][j] == 0 then self:_addZoneFrom(i, j, zones) end
      end
   end
...
=======
	...
	for j = 1, self._height do
		for i = 1, self._width do
			...
			if not changed and newMap[i][j] ~= self._map[i][j] then
				changed = true
			end
		end
	end
