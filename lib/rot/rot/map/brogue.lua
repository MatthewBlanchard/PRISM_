...
function Brogue:_buildFirstRoom(firstFloorBehavior)
   while true do
      ...
      elseif self._rng:random() < self._options.caveChance then
         ...
         local room =
            ROT.Map.BrogueRoom:createRandom(self._width, self._height, self._options, self._rng)
         if room:isValid(self._isWallCallback, self._canBeDugCallback) then
            table.insert(self._rooms, room)
            room:create(self._digCallback)
            self:_insertWalls(room._walls)
...
function Brogue:_generateRooms()
   ...
   for i = 1, 1000 do
      ...
      if self:_buildRoom(i > 375) then rooms = rooms + 1 end
   end
end
...
function Brogue:_buildRoom(forceNoCorridor)
   ...
   if d then
      if self._rng:random() < self._options.corridorChance and not forceNoCorridor then
         local cd
         if d[1] ~= 0 then
            cd = self._options.corridorWidth
         else
            cd = self._options.corridorHeight
         end
         local corridor = ROT.Map.Corridor:createRandomAt(
            x + d[1],
            y + d[2],
            d[1],
            d[2],
         ...
         if corridor:isValid(self._isWallCallback, self._canBeDugCallback) then
            ...
            local room =
               ROT.Map.BrogueRoom:createRandomAt(dx, dy, d[1], d[2], self._options, self._rng)
...
function Brogue:_generateLoops()
   ...
   local function pass(x, y) return m[x][y] ~= 1 end
   for _ = 1, 300 do
...
function Brogue:_buildRoom(forceNoCorridor)
	...
	if d then
		if self._rng:random() < self._options.corridorChance and not forceNoCorridor then
			...
			local corridor =
				ROT.Map.Corridor:createRandomAt(x + d[1], y + d[2], d[1], d[2], { corridorLength = cd }, self._rng)
			if corridor:isValid(self._isWallCallback, self._canBeDugCallback) then
				local dx = corridor._endX
				local dy = corridor._endY
...
function Brogue:_closeDiagonalOpenings() end

<<<<<<< HEAD
function Brogue:_digCallback(x, y, value) self._map[x][y] = value == 2 and 0 or value end
...
function Brogue:_canBeDugCallback(x, y)
	...
	local drs = ROT.DIRS.FOUR
	for i = 1, #drs do
		if self._map[x + drs[i][1]][y + drs[i][2]] == 0 then
			return false
		end
	end
	return true
