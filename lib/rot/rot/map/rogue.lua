...
<<<<<<< HEAD
   ...
   self._doors = {}
   self._options =
      { cellWidth = math.floor(width * 0.0375), cellHeight = math.floor(height * 0.125) }
   if options then
...
function Rogue:_connectRooms()
   ...
   repeat
      ...
      repeat
         ...
         if
            (ncgx > 0 and ncgx <= self._options.cellWidth)
            and (ncgy > 0 and ncgy <= self._options.cellHeight)
         then
            room = self._rooms[cgx][cgy]

            if #room.connections > 0 then
               if room.connections[1][1] == ncgx and room.connections[1][2] == ncgy then break end
            end
      ...
      for j = 1, ch do
         ...
         if #room.connections == 0 then
            ...
            repeat
               ...
               if newI > 0 and newI <= cw and newJ > 0 and newJ <= ch then
                  otherRoom = self._rooms[newI][newJ]
                  validRoom = true

                  if #otherRoom.connections == 0 then break end

                  for k = 1, #otherRoom.connections do
                     if otherRoom.connections[k][1] == i and otherRoom.connections[k][2] == j then
                        validRoom = false
                        break
                     end
                  end
            ...
            until #dirs < 1
            if validRoom then
               table.insert(room.connections, { otherRoom.cellx, otherRoom.celly })
            else
...
function Rogue:_createRandomRoomConnections() return end
   ...
   for i = 1, cw do
      for j = 1, ch do
         ...
         for k = 1, #room.connections do
            ...
            self:_drawCorridor(
               self:_getWallPosition(room, wall),
               self:_getWallPosition(otherRoom, otherWall)
            )
         end
      end
   end
...
function Rogue:_connectRooms()
	...
	repeat
		...
		repeat
			...
			if (ncgx > 0 and ncgx <= self._options.cellWidth) and (ncgy > 0 and ncgy <= self._options.cellHeight) then
				room = self._rooms[cgx][cgy]

				if #room.connections > 0 then
					if room.connections[1][1] == ncgx and room.connections[1][2] == ncgy then
						break
					end
				end
