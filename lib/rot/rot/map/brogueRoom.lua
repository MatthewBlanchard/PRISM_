...
<<<<<<< HEAD
   ...
   if doorX then self._doors:push(doorX, doorY) end
end
...
function BrogueRoom:getRandomWalkableTile()
   while true do
      ...
      if self:_coordIsFloor(rx, ry) then return rx, ry end
   end
end
...
=======
	...
	dims.x2 = math.floor(rng:random(dims.x1, (dims.x1 + dims.w1) - dims.w2))
	dims.y2 = math.floor(rng:random(dims.y1, (dims.y1 + dims.h1) - dims.h2))
	if dims.x2 ~= dims.x2 then
		dims.x2 = dims.x1
	end
	if dims.y2 ~= dims.y2 then
		dims.y2 = dims.y1
	end
   ...
   for x = left, right do
      for y = top, bottom do
         if self:_coordIsFloor(x, y) then
            if not isWallCallback(x, y) or not canBeDugCallback(x, y) then return false end
...
function BrogueRoom:_checkHorizontalEdge(isWallCallback, x, y)
   return self._walls:find(x, y) and self._walls:find(x - 1, y) and self._walls:find(x + 1, y)
   -- and isWallCallback(x - 1, y)
   -- and isWallCallback(x + 1, y)
end

function BrogueRoom:_checkVerticalEdge(isWallCallback, x, y)
   return self._walls:find(x, y) and self._walls:find(x, y - 1) and self._walls:find(x, y + 1)
...
function BrogueRoom:_coordIsWall(x, y)
	...
	for i = 1, #dirs do
		local dir = dirs[i]
		if self:_coordIsFloor(x + dir[1], y + dir[2]) then
			return true
		end
	end
	return false
...
function BrogueRoom:getLeft()
	return math.min(self._dims.x1, self._dims.x2)
end
function BrogueRoom:getRight()
	return math.max(self._dims.x1 + self._dims.w1, self._dims.x2 + self._dims.w2)
end
function BrogueRoom:getTop()
