...
<<<<<<< HEAD
   ...
   if doorX and doorY then self:addDoor(doorX, doorY) end
=======
	self._x1 = x1
	self._x2 = x2
	self._y1 = y1
...
<<<<<<< HEAD
function Room:addDoor(x, y) self._doors:push(x, y) end
=======
...
function Room:_checkHorizontalEdge(isWallCallback, x, y)
   ...
   return y == top or y == bottom and not isWallCallback(x, y + 1) and not isWallCallback(x, y - 1)
end

function Room:_checkVerticalEdge(isWallCallback, x, y)
   local left = self:getLeft() - 1
   local right = self:getRight() + 1
   return x == left or x == right and not isWallCallback(x + 1, y) and not isWallCallback(x - 1, y)
end
...
<<<<<<< HEAD
   ...
   local debugString = "room    : "
      .. (self._x1 and self._x1 or "not available")
      .. ","
...
=======
	...
	for x = left, right do
		for y = top, bottom do
			if x == left or x == right or y == top or y == bottom then
				if not isWallCallback(x, y) then
					return false
				end
			else
...
<<<<<<< HEAD
   return {
      math.ceil((self:getLeft() + self:getRight()) / 2),
      math.ceil((self:getTop() + self:getBottom()) / 2),
   }
=======
	return { math.ceil((self:getLeft() + self:getRight()) / 2), math.ceil((self:getTop() + self:getBottom()) / 2) }
>>>>>>> fbe4a4adf3bf1fc96ecb985cb65c5a009faf5ebc
end
...
=======
function Room:getLeft()
	return self._x1
end
--- Get right-most floor space.
-- @treturn int right-most floor
function Room:getRight()
	return self._x2
end
--- Get top most floor space.
-- @treturn int top-most floor
function Room:getTop()
	return self._y1
end
--- Get bottom-most floor space.
-- @treturn int bottom-most floor
function Room:getBottom()
	return self._y2
end
>>>>>>> fbe4a4adf3bf1fc96ecb985cb65c5a009faf5ebc
