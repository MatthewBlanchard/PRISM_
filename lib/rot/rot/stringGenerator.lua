...
function StringGenerator:init(options)
	self._options = { words = false, order = 3, prior = 0.001 }
...
function StringGenerator:_split(str) return str:split(self._options.words and " " or "") end

function StringGenerator:_join(arr) return table.concat(arr, self._options.words and " " or "") end

function StringGenerator:_observeEvent(context, event)
	local key = self:_join(context)
	if not self._data[key] then self._data[key] = {} end
	if not self._data[key][event] then self._data[key][event] = 0 end
...
function StringGenerator:_backoff(context)
	...
	if #ctx > self._options.order then
		while #ctx > self._options.order do
			table.remove(ctx, 1)
		end
	elseif #ctx < self._options.order then
		while #ctx < self._options.order do
			table.insert(ctx, 1, self._boundary)
		end
	end
