...
<<<<<<< HEAD
   ...
   assert(
      dice_notation == string.match(dice_notation, dice_pattern),
      "Dice string incorrectly formatted."
   )
...
function Dice:getTotalRerolls()
   return (self.is_reroll_plural and self.rerolls * self.num) or self.rerolls
end
...
--- Modifies bonus
function Dice:__add(value)
   self.bonus = self.bonus + value
   return self
=======
	-- If dice_notation is a number, we must convert it into the proper dice string format
	if type(dice_notation) == "number" then
		dice_notation = "1d" .. dice_notation
	end
...
--- Sets dice minimum result boundaries (if nil, no minimum result)
function Dice:setMin(value)
	self.minimum = value
end

--- Get number of total dice
function Dice:getNum()
	return self.num
end

--- Get number of total faces on a dice
function Dice:getFaces()
	return self.faces
end

--- Get bonus to be added to the dice total
function Dice:getBonus()
	return self.bonus
end

--- Get rerolls to be added to the dice
function Dice:getRerolls()
	return self.rerolls
end

--- Get number of total dice sets
function Dice:getSets()
	return self.sets
end

--- Get bonus to be added to all dice (if double bonus enabled) otherwise regular bonus
function Dice:getTotalBonus()
	return (self.is_bonus_plural and self.bonus * self.num) or self.bonus
end
...
--- Returns boolean that checks if all dice are to be rerolled together or individually
function Dice:isDoubleReroll()
	return self.is_reroll_plural
end

--- Returns boolean that checks if all dice are to apply a bonus together or individually
function Dice:isDoubleBonus()
	return self.is_bonus_plural
end
...
function Dice:__div(value)
<<<<<<< HEAD
   self.faces = self.faces + value
   return self
=======
	self.faces = self.faces + value
	return self
...
--- Modifies rerolls
function Dice:__pow(value)
<<<<<<< HEAD
   self.rerolls = self.rerolls + value
   return self
=======
	self.rerolls = self.rerolls + value
	return self
...
--- Modifies dice sets
function Dice:__mod(value)
<<<<<<< HEAD
   self.sets = self.sets + value
   return self
=======
	self.sets = self.sets + value
	return self
...
<<<<<<< HEAD
   local num_dice, dice_faces, bonus, is_bonus_plural, rerolls, is_reroll_plural, sets =
      self.num,
      self.faces,
      self.bonus,
      self.is_bonus_plural,
      self.rerolls,
      self.is_reroll_plural,
      self.sets
...
=======
	local num_dice, dice_faces, bonus, is_bonus_plural, rerolls, is_reroll_plural, sets =
		self.num, self.faces, self.bonus, self.is_bonus_plural, self.rerolls, self.is_reroll_plural, self.sets
...
<<<<<<< HEAD
   ...
   local bonus = ((str_b == "++" or str_b == "--") and "double")
      or ((str_b == "+" or str_b == "-") and "single")
      or nil
...
<<<<<<< HEAD
   ...
   for i = 1, sets do
      ...
      local total = 0
      for _, number in ipairs(rolls) do
         total = total + number
      end
      set_rolls[i] = total
   end
...
=======
	if type(self) ~= "table" then
		self = ROT.Dice:new(self, minimum):setRNG(rng)
	end
	local num_dice, dice_faces = self.num, self.faces
	local bonus, rerolls = self.bonus, self.rerolls
	local is_bonus_plural, is_reroll_plural = self.is_bonus_plural, self.is_reroll_plural
	...
	for i = 1, sets do
		...
		-- bonus gets added to the last roll if it is not plural
		if not is_bonus_plural then
			rolls[#rolls] = rolls[#rolls] + bonus
		end
