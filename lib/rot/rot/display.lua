...
<<<<<<< HEAD
   ...
   self.defaultForegroundColor = dfg
      or { 0.9215686274509803, 0.9215686274509803, 0.9215686274509803 }

   self.defaultBackgroundColor = dbg
   ...
   for i = 0, 255 do
      ...
      self.glyphs[i] = self.graphics.newQuad(
         ...
         sy,
         self.imageCharWidth,
         self.imageCharHeight,
         self.glyphSprite:getWidth(),
         self.glyphSprite:getHeight()
      )
   end
...
=======
	...
	self.defaultForegroundColor = dfg or { 0.9215686274509803, 0.9215686274509803, 0.9215686274509803 }

	self.defaultBackgroundColor = dbg or { 0.058823529411764705, 0.058823529411764705, 0.058823529411764705 }
...
<<<<<<< HEAD
   ...
   if subDisplay then
      assert(
         self.displays[subDisplay],
         "A sub display with name " .. subDisplay .. " does not exist."
      )
   ...
   for x = startX, endX do
      for y = startY, endY do
         ...
         if
            self.oldChars[x][y] ~= c
            or self.oldBackgroundColors[x][y] ~= bg
            or self.oldForegroundColors[x][y] ~= fg
         then
            self:_setColor(bg)
...
function Display:getWidth()
	return self:getWidthInChars()
end
function Display:getHeight()
	return self:getHeightInChars()
end
...
=======
function Display:getBackgroundColor(x, y)
	return self.backgroundColors[x][y]
end
>>>>>>> fbe4a4adf3bf1fc96ecb985cb65c5a009faf5ebc
...
=======
function Display:getForegroundColor(x, y)
	return self.foregroundColors[x][y]
end
>>>>>>> fbe4a4adf3bf1fc96ecb985cb65c5a009faf5ebc
	...
	while #tokens > 0 do -- interpret tokenized opcode stream
		...
		if token.type == ROT.Text.TYPE_TEXT then
			...
			for i = 1, #token.value do
				...
				-- TODO: ... so the next 15 lines or so do some pointless stuff
				-- Assign to `true` when the current char is full-width.
				isFullWidth = (cc > 0xff00 and cc < 0xff61) or (cc > 0xffdc and cc < 0xffe8) or cc > 0xffee
				-- Current char is space, whatever full-width or half-width both are OK.
				isSpace = c:byte() == 0x20 or c:byte() == 0x3000
				-- The previous char is full-width and
				-- current char is nether half-width nor a space.
