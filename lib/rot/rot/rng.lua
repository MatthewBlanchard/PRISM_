...
<<<<<<< HEAD
   ...
   if self.s0 < 0 then self.s0 = self.s0 + 1 end
   self.s1 = self.s1 - mash(self.seed)
   if self.s1 < 0 then self.s1 = self.s1 + 1 end
   self.s2 = self.s2 - mash(self.seed)
   if self.s2 < 0 then self.s2 = self.s2 + 1 end
   mash = nil
end
...
function RNG:getPercentage() return 1 + math.floor(self:getUniform() * 100) end
...
<<<<<<< HEAD
function RNG:getState() return { self.s0, self.s1, self.s2, self.c, self.seed } end
=======
...
<<<<<<< HEAD
   self.s0, self.s1, self.s2, self.c, self.seed = t[1], t[2], t[3], t[4], t[5]
end
