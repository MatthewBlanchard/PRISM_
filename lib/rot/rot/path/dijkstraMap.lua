...
<<<<<<< HEAD
   ...
   self._dirty = true
   if goalX and goalY then self:addGoal(goalX, goalY) end
=======
	DijkstraMap.super.init(self, goalX, goalY, passableCallback, options)
	self._map = Grid()
	self._goals = PointSet()
...
<<<<<<< HEAD
function DijkstraMap:hasGoal(x, y) return not not self._goals:find(x, y) end
=======
...
<<<<<<< HEAD
   if self._goals:push(x, y) then self._dirty = true end
   return self
=======
...
<<<<<<< HEAD
   if self._goals:prune(x, y) then self._dirty = true end
   return self
=======
...
=======
	local low = self._map:getCell(x, y)
	if not low or low == 0 or low == math.huge then
		return
	end
	local dir = nil
	for i = 1, topology or self._options.topology do
...
function DijkstraMap:_visitAdjacent(x, y)
   ...
   for i = 1, #self._dirs do
      ...
      local value = self._map:getCell(tx, ty) or self:_addCell(tx, ty, math.huge)

      low = math.min(low, value)
   end

   if self._map:getCell(x, y) > low + 2 then self._map:setCell(x, y, low + 1) end
end
...
function DijkstraMap:_visitAdjacent(x, y)
	if not self._passableCallback(x, y) then
		return
	end
