...
function PointSet:init() self._index = {} end

local function hash(x, y)
   return x and y * 0x4000000 + x or false --  26-bit x and y
end

function PointSet:find(x, y) return self._index[hash(x, y)] end

function PointSet:peek(i) return self[i], self[i + 1] end
...
local function iterate(self, i)
   i = i - 2
   if i > 0 then return i, self:peek(i) end
end

function PointSet:each() return iterate, self, #self + 1 end
