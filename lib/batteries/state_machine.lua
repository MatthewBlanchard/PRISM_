...
function state_machine:current_state() return self.states[self.current_state_name] end
...
function state_machine:in_state(name) return self.current_state_name == name end

function state_machine:has_state(name) return self.states[name] ~= nil end
...
function state_machine:add_state(name, state)
   if self:has_state(name) then
      ...
      if self:in_state(name) then self:_call_and_transition "enter" end
   end
...
function state_machine:replace_state(name, state, mask_transitions)
   ...
   if do_transitions then self:_call "exit" end
   self.states[name] = state
   if do_transitions then self:_call_and_transition("enter", self) end

   return self
end

--ensure a state doesn't exist; transition out of it if we're currently in it
function state_machine:clear_state(name) return self:replace_state(name, nil) end
...
function state_machine:reset()
   if self.reset_state_name then self:set_state(self.reset_state_name, true) end
end
...
function state_machine:update(dt) return self:_call_and_transition("update", dt) end

--draw the current state
function state_machine:draw() self:_call "draw" end
