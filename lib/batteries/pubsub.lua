...
function pubsub:_deferred() return self._defer_stack > 0 end
...
function pubsub:_defer_call(defer_f, event, callback)
	if not self:_deferred() then error "attempt to defer pubsub call when not required" end
...
function pubsub:_pop_defer(event)
	...
	if self._defer_stack < 0 then
		error(
			"pubsub defer stack underflow; don't call the defer methods directly - event reported: "
				.. tostring(event)
		)
	end
	if self._defer_stack == 0 then
...
function pubsub:unsubscribe(event, callback)
	...
	if callbacks then
		...
		if callbacks:size() == 0 then self.subscriptions[event] = nil end
	end
end

--check if there is a subscriber for a given event
function pubsub:has_subcriber(event) return self.subscriptions[event] ~= nil end
