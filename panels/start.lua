...
function Start:__new(display) Panel.__new(self, display, parent) end
...
function Start:draw()
   ...
   if self.fadeTime then
      ...
      if timeSinceFade > 2 then game.interface:pop() end
...
function Start:handleKeyPress(keypress)
   if self.fadeTime then game.interface:pop() end
...
function Start:draw()
	...
	local tC = math.max(0.09, 0.9 * t)
	self.display:clear(nil, nil, nil, nil, nil, nil, { 0.09, 0.09, 0.09, a })
	self.display:writeCenter("The Garden", math.floor(self.h / 2) - 10, { tC, tC, tC, a }, { 0.09, 0.09, 0.09, a })
	self.display:writeCenter(
		"Press any key to begin.",
		math.floor(self.h / 2),
		{ tC, tC, tC, a },
