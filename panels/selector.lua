...
local function bresenhamCallback(x, y) return game.level:getCellPassableNoActors(x, y) end
...
function SelectorPanel:draw()
   ...
   if not self.blink then
      self:write_plain(
         ...
         position.x,
         position.y,
         self.valid and SelectorPanel.blinkColor or SelectorPanel.invalidColor
      )
   end
   ...
   elseif self.action:getTargetObject(#self.targets + 1):is(targets.Point) then
      ...
      if #target == 0 then
         ...
         if message_system then
            message_system:add(game.level, "There are no targets.", game.curActor)
         end
         game.interface:pop()
         return
      end
...
function SelectorPanel:update(dt) self.blink = self.blinkFunc(dt) end
...
function SelectorPanel:updateTarget(target)
   ...
   local line, valid = Bresenham.line(
      game.curActor.position.x,
      game.curActor.position.y,
      self:getTargetPosition().x,
      self:getTargetPosition().y,
      bresenhamCallback
   )
   self.line = line
   self.valid = valid
...
function SelectorPanel:getValidTargets(index)
   ...
   if self.action.targets[index]:is(targets.Point) then return sight_component.seenActors end

   for i = 1, #sight_component.seenActors do
      if self.action:validateTarget(index, game.curActor, sight_component.seenActors[i]) then
         ...
         if not isTargeted then table.insert(targets, sight_component.seenActors[i]) end
      end
   end
...
function SelectorPanel:handleKeyPress(keypress)
   ...
   elseif
      self.action.targets[#self.targets + 1]:is(targets.Point)
      and self.movementTranslation[keypress]
   then
      self:moveTarget(self.movementTranslation[keypress])
   elseif keypress == "return" and self.valid then
      if
         self.action.targets[#self.targets + 1]:is(targets.Point)
         and self.action:validateTarget(#self.targets + 1, game.curActor, self.curTarget)
      then
