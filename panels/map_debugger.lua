...
function MapDebugger:__new(display, parent, level)
   ...
   if success == false then error(ret .. "\n" .. debug.traceback(self.coroutine)) end
end

function MapDebugger:update(dt) end
...
function MapDebugger:draw()
   ...
   for x = 1, viewX do
      for y = 1, viewY do
         ...
         self:write_plain(tile, {x=x, y=y}, { 1, 1, 1 })
      end
   end
end
...
function MapDebugger:handleKeyPress(keypress)
   ...
   if movement then self.offset = self.offset + movement * 10 end
