...
<<<<<<< HEAD
   ...
   if self.targetActor:hasComponent(components.Usable) then
      for k, action in pairs(self.targetActor.useActions) do
         if
            action:getNumTargets() > 0 and action:validateTarget(1, game.curActor, self.targetActor)
         then
   ...
   for k, action in pairs(game.curActor:getActions()) do
      if
         action:getNumTargets() > 0
         and action:validateTarget(1, game.curActor, self.targetActor)
         and not action:is(actions.Attack)
      then
         table.insert(self.allowedActions, action)
      end
   end
