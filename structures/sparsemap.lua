...
local function unhash(hash) return hash % 0x4000000, math_floor(hash / 0x4000000) end
=======
	return x and y * 0x4000000 + x --  26-bit x and y
end
...
function SparseMap:get(x, y) return self.map[hash(x, y)] or dummy end

function SparseMap:getByHash(hash) return self.map[hash] or dummy end
...
function SparseMap:count() return self.__count end
