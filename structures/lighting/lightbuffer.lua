...
function LightBuffer:getIndex(x, y)
	...
	assert(
		(y - 1) * self.w + (x - 1) < self.w * self.h,
		"Index out of bounds (" .. x .. ", " .. y .. ")"
	)
	return (y - 1) * self.w + (x - 1)
end

function LightBuffer:clear() ffi.fill(self.buffer, ffi.sizeof "LightColor" * self.w * self.h) end
...
function LightBuffer:getLight(x, y) return self:getColor(x, y) end
