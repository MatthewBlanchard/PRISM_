...
EquipCondition:afterAction(actions.Equip, function(self, level, actor, action)
   for k, effect in pairs(actor.effects) do
      action.owner:applyCondition(effect)
   end
end):where(Condition.ownerIsTarget)

EquipCondition:afterAction(actions.Unequip, function(self, level, actor, action)
   for k, effect in pairs(actor.effects) do
      action.owner:removeCondition(effect)
   end
end):where(Condition.ownerIsTarget)

EquipCondition:onAction(actions.Drop, function(self, level, actor, action)
   ...
   if not (equipper.slots[equipment.slot] == equipment) then return end
