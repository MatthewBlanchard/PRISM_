...
function CollideableDynamic:__new(size)
	assert(
		size and size >= 2,
		"Dynamic collideable must be at least 2 tiles long, use CollideableBox for 1 tile"
	)
	self.size = size
...
function CollideableDynamic:eachCell() return self.internalCollideable:eachCell() end
...
function CollideableDynamic:trySqueeze(level, actor, direction, rejected, accepted)
	...
	if self.internalCollideable:is(CollideableSnake) then
		return self.internalCollideable:moveCandidate(level, actor, direction),
			actor.position + direction
	end

	local snake_collideable, new_origin =
		self:createSnakeSpiral(actor, direction, rejected, accepted)
	return snake_collideable:moveCandidate(level, { position = new_origin }, direction),
...
function CollideableDynamic:acceptedSqueeze(level, actor, direction, rejected, accepted)
	...
	local snake_collideable, new_origin =
		self:trySqueeze(level, actor, direction, rejected, accepted)
