...
function Usable:getUseAction(action)
   for k, v in pairs(self.useActions) do
      if v:is(action) then return v end
   end
end
...
function Usable:addUseAction(action)
   ...
   if not self.defaultUseAction then self.defaultUseAction = action end
end
...
function Usable:removeUseAction(action)
   for k, v in pairs(self.useActions) do
      if v:is(action) then
         ...
         if self.defaultUseAction == v then self.defaultUseAction = nil end
         return
      end
   end
end
