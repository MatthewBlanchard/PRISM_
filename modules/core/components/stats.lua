...
function Stats.getStatBonus(actor, stat)
	...
	for k, cond in pairs(actor:getConditions()) do
		if cond[validStats[stat]] then condmods = condmods + cond[validStats[stat]](cond, actor) end
	end
...
function Stats.getAC(actor)
	local condmods = 0
	for k, cond in pairs(actor:getConditions()) do
		if cond.getAC then condmods = condmods + cond:getAC(actor) end
	end
...
function Stats.getMaxHP(actor)
	local condmods = 0
	for k, cond in pairs(actor:getConditions()) do
		if cond.getMaxHP then condmods = condmods + cond:getMaxHP(actor) end
	end

	return actor.maxHP + condmods
end

function Stats.getHP(actor) return actor.HP end

function Stats.setHP(actor, hp) actor.HP = math.max(0, math.min(actor.maxHP, hp)) end
