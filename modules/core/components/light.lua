...
local function randBiDirectional() return (math.random() - 0.5) * 2 end
...
function Light:__new(options)
   ...
   if options.effect then self.effect = options.effect[1](self.color, unpack(options.effect[2])) end
   self.falloff = options.falloff or 0.4
end

function Light:initialize(actor) self.color = LightColor(self.color.r, self.color.g, self.color.b) end
